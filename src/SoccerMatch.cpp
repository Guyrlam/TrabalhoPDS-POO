...
void SoccerMatch::nextRound(std::vector<std::string> *lines, int *cru_score, int *cam_score)
{
	...
	if (round % 2 != 0)
	{
		...
		lineText << "O time " << teamAName << " esta atacando";
		...
		{
			lineText << "Time " << teamAName << " perdeu a bola";
			lines->push_back(lineText.str());
			this->teamA->updateResistanceTeam(PLAYERS_PER_TEAM, 0.2);
		}
		else
		{
			lineText << "Time " << teamAName << " comecou perdendo a bola";
			lines->push_back(lineText.str());
		}
	}
	else
	{
		lineText << "O time " << teamBName << " esta atacando";
		lines->push_back(lineText.str());
		// Realizando todos os confrontos
		for (int i = 0; i < PLAYERS_PER_TEAM; i++)
		{
		...
		{
			lineText << "Time " << teamBName << " perdeu a bola";
			lines->push_back(lineText.str());
